...
/*
	...
	*/	
	...
	@PostMapping
	public ResponseEntity<String> addEmployee(@Valid @RequestBody Employee emp, BindingResult result) throws EmployeeException {
		ResponseEntity<String> response = null;
		
		

	Integer id = emp.getDeptId();
	dId = id;
     ...
     empService.validate(emp); // Calling Validation for Validate the entries	
		if (result.hasErrors()) {
			StringBuilder errMsg = new StringBuilder();
			for (FieldError err : result.getFieldErrors()) {
				errMsg.append(err.getDefaultMessage() + ",");
			}
			 response = new ResponseEntity<>(errMsg.toString(),HttpStatus.OK);
		} else {
